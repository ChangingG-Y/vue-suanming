<template>
  <div class="profile-page">
    ...
    <div class="profile-card" :class="{ partner: viewPartner }">
      ...
      <div class="stats-row">
        <div class="stat-pill">
          <div class="stat-num">{{ currentProfile.height ? currentProfile.height : '—' }}</div>
        ...
        <div class="stat-pill" :class="{ clickable: !viewPartner }" @click="!viewPartner && openWeightInput()">
          <div class="stat-num" :class="viewPartner ? 'weight-partner' : 'weight-num'">
            {{ currentProfile.currentWeight ? currentProfile.currentWeight : (viewPartner ? '—' : '记录') }}
          </div>
          <div class="stat-unit">
            kg · 体重
    ...
    <div class="section-card">
      ...
      <div v-if="currentWeights.length > 1" class="chart-wrap">
        <svg class="weight-chart" :viewBox="`0 0 ${chartW} ${chartH}`" preserveAspectRatio="none">
          ...
          <line v-for="gy in [padT, padT+(chartH-padT-padB)/2, chartH-padB]" :key="gy" :x1="padL" :y1="gy" :x2="chartW-padR" :y2="gy" stroke="#f0f0f5" stroke-width="1" />
          <path :d="chartAreaPath" :fill="`url(#${viewPartner ? 'wgp' : 'wgm'})`" />
          <path :d="chartLinePath" fill="none" :stroke="lineColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
          <circle v-for="(pt, i) in chartPoints" :key="i" :cx="pt.x" :cy="pt.y" r="4.5"
            fill="#fff" :stroke="lineColor" stroke-width="2.5" style="cursor:pointer"
            @click="tappedWeightIdx = tappedWeightIdx === i ? null : i" />
          <template v-if="tappedWeightIdx !== null && chartPoints[tappedWeightIdx]">
            <rect :x="clamp(chartPoints[tappedWeightIdx].x - 32, padL, chartW-padR-64)" :y="Math.max(chartPoints[tappedWeightIdx].y - 30, 2)" width="64" height="22" rx="7" :fill="lineColor" />
            <text :x="clamp(chartPoints[tappedWeightIdx].x, padL+32, chartW-padR-32)" :y="Math.max(chartPoints[tappedWeightIdx].y - 14, 16)" text-anchor="middle" fill="#fff" font-size="11" font-weight="700">
              {{ currentWeights[tappedWeightIdx].weight }} kg
            </text>
          </template>
        </svg>
      ...
      <div v-else-if="currentWeights.length === 1" class="empty-chart">
        <div class="solo-weight" :style="{ color: lineColor }">{{ currentWeights[0].weight }}<span class="solo-unit"> kg</span></div>
    ...
    <div class="section-card">
      ...
      <div class="calendar-grid">
        ...
        >
          ...
          <span v-if="day.weight" class="cal-kg" :class="{ 'partner-kg': viewPartner }">{{ day.weight }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="viewPartner && partnerWeights.length > 0" class="section-card">
      ...
      <div class="weight-list">
        <div v-for="r in [...partnerWeights].reverse().slice(0,10)" :key="r.id" class="wt-row">
          ...
          <span class="wt-val" style="color:#5b8af0">{{ r.weight }}<span class="wt-unit"> kg</span></span>
        </div>
      </div>
    </div>
    ...
    <van-popup v-model:show="showWeightModal" round position="bottom" safe-area-inset-bottom>
      ...
      <div class="sheet-inner">
        ...
        <div class="form-field">
          ...
          <input v-model="weightForm.weight" class="form-input" type="number" step="0.1" placeholder="如 68.5" />
        </div>
        <div class="form-field">
...
async function onAvatarChange(e) {
  ...
  try {
    ...
    myProfile.value.avatarUrl = url + '?t=' + Date.now(); showToast('头像已更新 ✓')
...
.today-num-partner { background: #5b8af0; color: #fff; border-radius: 50%; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800; line-height: 1; }
